...
class ParamsParser(argparse.ArgumentParser):
    def __init__(self, model, device):
        ...
        if model == 'FNN':
            ...
            self.add_argument('--dropout', type=float, default=0.354)

        if model == 'LSTM':
            self.add_argument('--learning_rate', type=float, default=0.006)
            self.add_argument('--batch_size', type=int, default=16)
            self.add_argument('--train_epochs', type=int, default=25)
            self.add_argument('--hidden_size', type=int, default=29)
            self.add_argument('--num_layers', type=int, default=4)
            self.add_argument('--dropout', type=float, default=0.365)
        ...
        if model == 'Transformer':
            ...
            self.add_argument('--dropout', type=float, default=0.104)

        if model == 'iTransformer':
            self.add_argument('--learning_rate', type=float, default=0.000921)
            self.add_argument('--batch_size', type=int, default=16)
            self.add_argument('--train_epochs', type=int, default=25)
            possible_d_models = [d for d in range(8, 513) if d % 8 == 0]
